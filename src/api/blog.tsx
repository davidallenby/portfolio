...
import type { BlogPost, BlogPostCategory, BlogPostTag, GetBlogPostsPayload } from '@interfaces/blog.interfaces';
import type { WPBlogPost, WpCategory, WpTag } from '@interfaces/wordpress.interfaces';
 ...
 * @returns
 */
export const getBlogPosts = async (payload: GetBlogPostsPayload): Promise<BlogPost[]> => {
	try {
...
const mapWpItemsAsClient = (item: WpTagOrWpCategory): BlogPostTag | BlogPostCategory => {
 ...
 * @returns
 */
export const getBlogPostBySlug = async (slug: string): ApiResponse<BlogPost> => {
	try {
