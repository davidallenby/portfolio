...
import type { BlogPost, BlogPostTag } from '@interfaces/blog.interfaces';
...
const BlogPostListItem: FC<BlogPostListItemProps> = ({ post, loading = false }) => {
	const router = useRouter();

	const openPostUrl = (e: React.MouseEvent<HTMLDivElement, globalThis.MouseEvent>) => {
		if (loading) {
			return;
		}
	...
	const setCategoryTagTemplate = (tagIds: number[]) => {
		...
		return active.map((tag: BlogPostTag, i: number) => {
			return (
				<Chip key={i} small className="my-2 me-3">
					{tag.name}
				</Chip>
			);
		});
	...
	return (
		<div
			...
			onClick={openPostUrl}
		>
			<BlogPostListItemImage href={post.url} src={post.featuredImageUrl} loading={loading} />
			<div className="BlogPostListItem__content me-sm-5">
				<h3 className="BlogPostListItem__title mb-1">
					<Link className="BlogPostListItem__title text-decoration-none" href={post.url}>
						{loading ? <Skeleton /> : post.title}
					</Link>
				</h3>
				<p className="mb-2">
					<small>{loading ? <Skeleton width={130} /> : getDateString(post.dateCreated)}</small>
				</p>
				{loading ? (
					<>
						<p className="mb-2">
				...
				{loading ? <Skeleton width={130} height={28.8} /> : setCategoryTagTemplate(post.tags)}
			</div>
		</div>
	);
};
