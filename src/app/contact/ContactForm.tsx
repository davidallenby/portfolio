...
const ContactForm: FC = () => {
	...
	return (
		<div className="ContactForm">
			{!sending && !success && (
				<form onSubmit={handleSubmit(onSubmit)} className="position-relative">
					<div className="mb-4">
						...
						/>
						{errors.name && <span className="d-block text-danger mt-1">{errors.name.message}</span>}
					</div>
					...
					<div className="mb-4">
						...
						/>
						{errors.email && <span className="d-block text-danger mt-1">{errors.email.message}</span>}
					</div>
					...
					<div className="mb-4">
						...
						></textarea>
						{errors.message && <span className="d-block text-danger mt-1">{errors.message.message}</span>}
					</div>

					{error && (
						<div className="p-3 border border-danger bg-light mb-3">
					...
					<div>
						...
						<button disabled={sending} type="submit" className="btn btn-outline-primary">
							Submit
						</button>
					</div>
				</form>
			...
			{sending && (
				<div className="position-absolute top-0 left-0 h-100 w-100 ContactForm__overlay">
					<SendingSpinner />
				</div>
			)}

			{!sending && success && (
				<div className="position-absolute top-0 left-0 h-100 w-100 ContactForm__overlay pt-5 text-center">
